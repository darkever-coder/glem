...
# NOTE: Do NOT use set -e here. Package installs may return
# non-zero on warnings/conflicts and we handle errors ourselves.
...
spinner() {
    ...
    local i=0
    local elapsed=0
    local timeout=600  # 10 min max per task
    while kill -0 "$pid" 2>/dev/null; do
        ...
        if [ $elapsed -ge $((timeout * 5)) ]; then
            ...
            printf "\r  ${RED}✗${NC} %s ${RED}(timeout)${NC}     \n" "$message"
            return 0
        fi
    done
    wait "$pid" 2>/dev/null
    ...
    if [ $exit_code -eq 0 ]; then
        ...
        printf "\r  ${RED}✗${NC} %s ${RED}(failed, continuing)${NC}     \n" "$message"
    fi
    return 0  # always return 0 so the script never aborts
}
...
step_gpu() {
    ...
    # NOTE: We do NOT install vulkan-loader-android separately.
    # The mesa-vulkan-icd-* packages already pull in a compatible
    # vulkan loader. Installing vulkan-loader-android on top causes
    # a dpkg conflict with vulkan-loader-generic and crashes.
...
# ============== STEP 12: INSTALL POSTGRESQL + DB TOOLS ==============
step_database() {
    update_progress
    echo -e "${PURPLE}[Step ${CURRENT_STEP}/${TOTAL_STEPS}] Installing PostgreSQL + Database Tools...${NC}"
    echo ""

    install_pkg "postgresql" "PostgreSQL Server"
    install_pkg "libpq" "PostgreSQL Client Library"

    # Initialize database cluster if not already done
    if [ ! -d "$PREFIX/var/lib/postgresql" ]; then
        echo -e "  ${YELLOW}⏳${NC} Initializing PostgreSQL database..."
        mkdir -p "$PREFIX/var/lib/postgresql"
        initdb -D "$PREFIX/var/lib/postgresql" > /dev/null 2>&1 || true
        echo -e "  ${GREEN}✓${NC} Database cluster initialized"
    else
        echo -e "  ${GREEN}✓${NC} Database cluster already exists"
    fi

    # Install pgcli (smart terminal client with autocomplete)
    echo -e "  ${YELLOW}⏳${NC} Installing pgcli (smart Postgres CLI)..."
    pip install pgcli > /dev/null 2>&1 || true
    echo -e "  ${GREEN}✓${NC} pgcli installed"

    # Create convenience scripts
    cat > ~/pg-start.sh << 'PGSTARTEOF'
#!/data/data/com.termux/files/usr/bin/bash
echo "Starting PostgreSQL..."
pg_ctl -D "$PREFIX/var/lib/postgresql" -l "$PREFIX/var/lib/postgresql/server.log" start
echo "PostgreSQL running. Connect with: pgcli"
PGSTARTEOF
    chmod +x ~/pg-start.sh

    cat > ~/pg-stop.sh << 'PGSTOPEOF'
#!/data/data/com.termux/files/usr/bin/bash
...
PGSTOPEOF
    ...
    echo -e "  ${GREEN}✓${NC} Created ~/pg-start.sh and ~/pg-stop.sh"
    echo ""
    echo -e "  ${CYAN}NOTE: DBeaver won't run on Termux (needs desktop JDK).${NC}"
    echo -e "  ${CYAN}Use pgcli (terminal), or VS Code Database extensions.${NC}"
    echo -e "  ${CYAN}Start Postgres:  bash ~/pg-start.sh${NC}"
    echo -e "  ${CYAN}Connect:         pgcli${NC}"
}

# ============== STEP 13: INSTALL WINE ==============
step_wine() {
    update_progress
...
# ============== STEP 15: QUALITY OF LIFE PACKAGES ==============
step_qol() {
    update_progress
    echo -e "${PURPLE}[Step ${CURRENT_STEP}/${TOTAL_STEPS}] Installing Quality-of-Life packages...${NC}"
    echo ""

    # Terminal essentials
    install_pkg "tmux" "tmux (terminal multiplexer)"
    install_pkg "bash-completion" "Bash Completion"
    install_pkg "htop" "htop (system monitor)"
    install_pkg "fastfetch" "Fastfetch (system info)"

    # Termux-Android bridge
    install_pkg "termux-api" "Termux:API (camera, SMS, sensors)"

    # File & cloud tools
    install_pkg "rclone" "rclone (cloud storage mount)"
    install_pkg "rsync" "rsync (file sync)"
    install_pkg "zip" "zip"
    install_pkg "unzip" "unzip"
    install_pkg "p7zip" "7zip"

    # Desktop polish
    install_pkg "tumbler" "Tumbler (thumbnail previews)"
    install_pkg "xfce4-whiskermenu-plugin" "Whisker Menu (Start Menu)"
    install_pkg "xfce4-notifyd" "Desktop Notifications"
    install_pkg "dbus" "D-Bus (desktop messaging)"
    install_pkg "evince" "Evince (PDF viewer)"
    install_pkg "xarchiver" "Xarchiver (archive manager)"
    install_pkg "pavucontrol" "PulseAudio Volume Control"

    # Setup termux-storage so Termux can see /sdcard
    echo -e "  ${YELLOW}⏳${NC} Linking phone storage..."
    termux-setup-storage < /dev/null > /dev/null 2>&1 || true
    echo -e "  ${GREEN}✓${NC} Storage linked (~/storage)"

    # Add useful aliases to bashrc
    if ! grep -q "# GLEM aliases" ~/.bashrc 2>/dev/null; then
        cat >> ~/.bashrc << 'ALIASEOF'
...
ALIASEOF
        echo -e "  ${GREEN}✓${NC} Shell aliases added"
    fi

    echo -e "  ${GREEN}✓${NC} QOL packages installed"
}

# ============== STEP 16: WINDOWS 11 THEME ==============
step_theme() {
    update_progress
    echo -e "${PURPLE}[Step ${CURRENT_STEP}/${TOTAL_STEPS}] Installing Windows 11 Theme...${NC}"
    echo ""

    install_pkg "sassc" "Sass Compiler (for themes)"

    # GTK + Window Manager theme
    echo -e "  ${YELLOW}⏳${NC} Downloading Win11 GTK theme..."
    cd ~
    rm -rf Win11-gtk-theme 2>/dev/null
    (git clone --depth 1 https://github.com/yeyushengfan258/Win11-gtk-theme.git > /dev/null 2>&1) &
    spinner $! "Cloning Win11-gtk-theme..."

    if [ -d ~/Win11-gtk-theme ]; then
        cd ~/Win11-gtk-theme
        (bash ./install.sh -c dark --tweaks round > /dev/null 2>&1) &
        spinner $! "Installing Win11 Dark theme..."
        # Also install light variant
        (bash ./install.sh -c light --tweaks round > /dev/null 2>&1) &
        spinner $! "Installing Win11 Light theme..."
    else
        echo -e "  ${RED}✗${NC} Failed to clone Win11-gtk-theme"
    fi

    # Icon theme
    cd ~
    rm -rf Win11-icon-theme 2>/dev/null
    (git clone --depth 1 https://github.com/yeyushengfan258/Win11-icon-theme.git > /dev/null 2>&1) &
    spinner $! "Cloning Win11-icon-theme..."

    if [ -d ~/Win11-icon-theme ]; then
        cd ~/Win11-icon-theme
        (bash ./install.sh > /dev/null 2>&1) &
        spinner $! "Installing Win11 icons..."
    else
        echo -e "  ${RED}✗${NC} Failed to clone Win11-icon-theme"
    fi

    # Auto-apply theme on first boot via xfce settings
    mkdir -p ~/.config/xfce4/xfconf/xfce-perchannel-xml
    cat > ~/.config/xfce4/xfconf/xfce-perchannel-xml/xsettings.xml << 'XSETEOF'
...
XSETEOF
    ...
    cat > ~/.config/xfce4/xfconf/xfce-perchannel-xml/xfwm4.xml << 'XFWMEOF'
<?xml version="1.0" encoding="UTF-8"?>
<channel name="xfwm4" version="1.0">
  <property name="general" type="empty">
    <property name="theme" type="string" value="Win11-Dark"/>
  </property>
</channel>
XFWMEOF

    echo -e "  ${GREEN}✓${NC} Win11 theme set as default (dark mode)"

    # Clean up source folders to save space
    rm -rf ~/Win11-gtk-theme ~/Win11-icon-theme 2>/dev/null
    echo -e "  ${GREEN}✓${NC} Cleaned up theme source files"
...
show_completion() {
    ...
    echo -e "   Theme:      Windows 11 Dark (with icons + Whisker Menu)"
    echo -e "   Apps:       Firefox, VS Code, Evince, Xarchiver, Thunar"
    echo -e "   Dev:        Node.js, Git, Python, .NET 8 SDK, Make, Clang"
    echo -e "   Database:   PostgreSQL + pgcli (bash ~/pg-start.sh)"
    echo -e "   Proot:      Ubuntu (proot-distro login ubuntu)"
    echo -e "   Sudo:       fake sudo wrapper (no real root)"
    echo -e "   QOL:        tmux, htop, rclone, rsync, Termux:API, dbus"
    ...
    echo -e "   ${GREEN}ubuntu${NC}                 Enter Ubuntu shell (alias)"
    echo -e "   ${GREEN}tmux${NC}                   Terminal multiplexer"
    echo -e "   ${GREEN}htop${NC}                   System monitor"
    echo -e "   ${GREEN}rclone config${NC}          Setup cloud storage"
    echo -e "   ${GREEN}bash ~/pg-start.sh${NC}     Start PostgreSQL"
    echo -e "   ${GREEN}pgcli${NC}                  Connect to PostgreSQL"
    ...
    echo -e "${WHITE}  TIP: Install Termux:API app from F-Droid for camera/SMS/GPS${NC}"
    echo ""
}
