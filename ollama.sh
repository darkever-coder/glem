...
# ---- Step 1b: Remove existing ollama ----
echo ""
echo -e "${YELLOW}[1b] Removing existing ollama...${NC}"

# Kill any running ollama process
if pgrep -f "ollama" > /dev/null 2>&1; then
    echo "  Stopping running ollama server..."
...
# Uninstall pkg version (keeps models intact)
if dpkg -l ollama > /dev/null 2>&1; then
    echo "  Uninstalling pkg version of ollama..."
    pkg uninstall ollama -y > /dev/null 2>&1
    echo -e "${GREEN}  pkg ollama removed.${NC}"
elif [ -f "$PREFIX/bin/ollama" ]; then
    echo "  Found custom ollama binary, will be overwritten."
else
    echo "  No existing ollama found."
fi

# Clean stale binary if still there
rm -f "$PREFIX/bin/ollama" 2>/dev/null

# Note: models in ~/.ollama/models are preserved
echo -e "${GREEN}  Cleanup done. Your downloaded models are safe.${NC}"
